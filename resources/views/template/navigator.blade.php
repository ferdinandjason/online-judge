...
<div class="nav" id="navigator" role="navigation" style="background: white">
    <div class="ui blue secondary pointing menu" style="{{(Route::current()->getName() == 'welcome')?'background:rgba(44,49,65,.80)':''}}" id={{ isset($isIndex) ? 'navi' : 'n' }}>
        <div class="ui container">
            ...
            @endif
            <a href="{{route('problem.index')}}" class="ui item{{(\Request::is('problems*') || \Request::is('tag*'))?' active':''}}"><i class="fas fa-file-code" style="color: blue"></i>&nbsp;&nbsp;Problem</a>
            <a href="{{route('submission.index')}}" class="ui item{{(\Request::is('submissions*'))?' active':''}}" style="{{(\Request::is('submissions'))?'color:red !important':''}}"><i class="far fa-thumbs-up" style="color: red"></i>&nbsp;&nbsp;Submission</a>
